...
def extract_vn_data(browser, wait_time=10):
    ...
    replace_dict = {'PM-10':'PM10', 'PM-2-5':'PM2.5','Ngày giờ':'datetime', 'VN_AQI giờ':'VN_AQI' }
...
def extract_vn_stations(browser, url= 'http://enviinfo.cem.gov.vn/', wait_time=10):
...
def select_vn_station(browser, station,wait_time=10):
    ...
    """
    # get the search box 
    inputelement = browser.find_element_by_xpath('//*[@id="cbbStations_chosen"]/div/div/input')
...
def extract_station_info(browser):
    ...
    city = browser.find_elements_by_xpath('//*[@id="station_province"]')[0].text.split(': ')[-1]
    address = browser.find_elements_by_xpath('//*[@id="station_address"]')[0].text.split(': ')[-1]
    long, lat = browser.find_elements_by_xpath('//*[@id="longlat"]')[0].text.split(' - ')
    long = long.split(': ')[-1]
    lat = lat.split(': ')[-1]

    return {'city_vn': city, 'address': address, 'longitude':long, 'latitude':lat}


def download_vn_data(url='http://enviinfo.cem.gov.vn/', save_folder='../data/vn_epa/'):
