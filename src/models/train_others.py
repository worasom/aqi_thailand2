...
def train_city_s2(
    ...
    data, model, fire_cols = load_model1(
        city='Chiang Mai', pollutant='PM2.5', split_list=[
            0.45, 0.25, 0.3])

    if (len(x_cols_org) != 0) and (lag_dict is not None):
    ...
    model = get_nn_model(
        input_shape=xtrn.shape[1],
        output_shape=ytrn.shape[1],
        num_layer=nn_dict['num_layer'],
        nn_size=nn_dict['nn_size'],
        act_fun=nn_dict['act_fun'],
        drop=nn_dict['drop'],
        lr=nn_dict['lr'],
        momentum=nn_dict['momentum'])
    esm = EarlyStopping(patience=8, verbose=0, restore_best_weights=True)
    history = model.fit(
        xtrn,
        ytrn,
...
def train_city_s1_v2(
    ...
    if lag_dict is None:
        ...
        data.lag_dict, data.fire_dict, gp_result = op_lag_fire(
    ...
    show_fea_imp(
        feat_imp,
        filename=data.report_folder +
        f'{poll_name}_rf_fea_op2_nolag.png',
        title='rf feature of importance(final)')
